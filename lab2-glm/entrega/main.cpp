...
// Diz se a câmera deve se mover
//      -1.0f : move na direção negativa (em direção a -X ou -Z)
//       0.0f : não se move
//       1.0f : se move na direção positiva (em direção a X ou Z)
float g_CameraMoveX = 0.0f;
float g_CameraMoveZ = 0.0f;

// Inicializa as coordenadas da câmera
...
int main()
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00342978 - Sandro Rudiero Saibro Viégas", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        g_CameraPosX += 0.01f * (g_CameraMoveX*x + g_CameraMoveZ*(right.x));
        g_CameraPosY += 0.01f * (g_CameraMoveX*y + g_CameraMoveZ*(right.y));
        g_CameraPosZ += 0.01f * (g_CameraMoveX*z + g_CameraMoveZ*(right.z));

        glm::vec4 camera_position_c = glm::vec4(g_CameraPosX, g_CameraPosY, g_CameraPosZ, 1.0f);
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mod)
{
    ...
    // Lógica de movimento da câmera para a questão 4.
    // A tecla 'W' deve movimentar a câmera para FRENTE.
    // A tecla 'S' deve movimentar a câmera para TRÁS.
    // A tecla 'D' deve movimentar a câmera para DIREITA.
    // A tecla 'A' deve movimentar a câmera para ESQUERDA.
