...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 342978 - Sandro Rudiero Saibro Viégas", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        PushMatrix(model);
            ...
            PushMatrix(model);
                model = model 
                      * Matrix_Scale(0.5f, -0.4f, 2.0f) 
                      * Matrix_Translate(0.0f, -0.15f, 0.0f)
        ...
        PushMatrix(model);
            ...
            PushMatrix(model);
                ...
                PushMatrix(model);
                    ...
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform); // #### BRAÇO DIREITO
                PopMatrix(model);
                PushMatrix(model);
                    ...
                    PushMatrix(model);
                        model = model * Matrix_Scale(0.2f, 0.6f, 0.2f);
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO
                        PushMatrix(model);
                            model = model 
                                  * Matrix_Translate(0.0f, -1.1f, 0.0f)
                                  * Matrix_Scale(1.0f, 0.2f, 1.0f);
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model);
            ...
            PushMatrix(model);
                model = model * Matrix_Translate(-0.75f, -1.75f, 0.0f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform); // #### PERNA SUPERIOR DIREITA
                PushMatrix(model);
                    model = model 
                          * Matrix_Translate(0.0f, -1.1f, 0.0f)
                          * Matrix_Scale(0.8f, 1.2f, 0.8f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform); // #### PERNA INFERIOR DIREITA
                    PushMatrix(model);
                        model = model 
                          * Matrix_Translate(0.0f, -1.1f, 0.5f)
                          * Matrix_Scale(1.0f, 0.1f, 2.0f);
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
